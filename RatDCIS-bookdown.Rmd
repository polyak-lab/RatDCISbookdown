...
# 3. ep
EpddsGrowth=Epdds
design(EpddsGrowth)=~GrowthRate+factor(Batch)  ## error is here!
...
# 3. Cd45
CDddsGrowth=CDdds
design(CDddsGrowth)=~GrowthRate+factor(Batch)
...
DNddsGrowth=DNdds
design(DNddsGrowth)=~(GrowthRate)+factor(Batch)
...
DNa1=results(DNddsGrowth, name="GrowthRate") #DNComp4$stable_vs_growing
DNa1$Gene=rownames(DNa1)
DNa=DNa1[ which(DNa1$padj<0.05 & DNa1$baseMean>80 & abs(DNa1$log2FoldChange)>0.15), ]
with(DNa1, plot(log2FoldChange, -log10(padj), pch=20, main="Volcano plot: DN growing (+) vs stable (-)", cex=1.0, xlab=bquote(~Log[2]~fold~change), ylab=bquote(~-log[10]~Q~value)))
with(subset(DNa1, padj<0.05 & abs(log2FoldChange)>0.15 & baseMean>80), points(log2FoldChange, -log10(padj), pch=20, col="red", cex=0.5))
with(subset(DNa1, padj<0.05 & abs(log2FoldChange)>0.15 & baseMean>80), text(log2FoldChange+0.05, -log10(padj)+0.05, Gene, pch=20, col="red", cex=0.75))
vstDN$GRcutoff=cut(vstDN$GrowthRate, c(-1, 2, 5, 10, 20), c("L", "M1", "O1", "O2") )

colnames(vstDN)=vstDN$TumorIDnew
heatmap.2(assay(vstDN)[ DNa$Gene, ], scale="row", trace="none", ColSideColors =brewer.pal(4, "Greens")[ (as.numeric(factor(vstDN$GRcutoff)))], col=RdBu[11:1], main="DN genes")
...
CDa1=results(CDddsGrowth, name="GrowthRate")
CDa1$Gene=rownames(CDa1)
#CDa1=CDComp4$stable_vs_growing
CDa=CDa1[ which(CDa1$padj<0.05 & CDa1$baseMean>50 & abs(CDa1$log2FoldChange)>0.15), ]
lx2=which(CDa$Gene%in%RatAllImm)
...
with(subset(CDa1, padj<0.05 & abs(log2FoldChange)>0.15 & baseMean>50 ), points(log2FoldChange, -log10(padj), pch=20, col="red", cex=0.5))
with(subset(CDa1, padj<0.05 & abs(log2FoldChange)>0.15 & baseMean>50 ), text(log2FoldChange+0.02, -log10(padj)+0.05, Gene, pch=20, col="red", cex=0.75))
...
```{r }
vstCD$GRcutoff=cut(vstCD$GrowthRate, c(-1, 2, 5, 10, 20), c("L", "M1", "O1", "O2") )

colnames(vstCD)=vstCD$TumorIDnew

heatmap.2(assay(vstCD)[ CDa$Gene[lx2], ], scale="row", trace="none", ColSideColors =brewer.pal(4, "Greens")[ (as.numeric(factor(vstCD$GRcutoff)))], col=RdBu[11:1], main="CD genes")
...
Epa1=results(EpddsGrowth, name="GrowthRate")
Epa1$Gene=rownames(Epa1)

Epa=Epa1[ which(Epa1$padj<0.02 & Epa1$baseMean>80 & abs(Epa1$log2FoldChange)>0.25), ]
...
with(subset(Epa1, padj<0.05 & abs(log2FoldChange)>0.25), points(log2FoldChange, -log10(padj), pch=20, col="red", cex=0.5))
with(subset(Epa1, padj<0.05 & abs(log2FoldChange)>0.25), text(log2FoldChange+0.05, -log10(padj)+0.1, Gene, pch=20, col="red", cex=0.75))

vstEp$GRcutoff=cut(vstEp$GrowthRate, c(-1, 2, 5, 10, 20), c("L", "M1", "O1", "O2") )

colnames(vstEp)=vstEp$TumorIDnew

#EpaB=Epa[order(1/abs(Epa$log2FoldChange), Epa$padj), ]

heatmap.2(assay(vstEp)[ Epa$Gene, ], scale="row", trace="none", ColSideColors = brewer.pal(4, "Greens")[ (as.numeric(factor(vstEp$GRcutoff)))], col=RdBu[11:1], main="Ep genes")
...
design(EpddsInflam)=~GrowthRate
EpddsInflam=DESeq(EpddsInflam)

ResA=results(EpddsInflam, name="GrowthRate") #contrast = c("Growth", "stable", "growing"))
ResAb=ResA[which(ResA$padj<0.05 & abs(ResA$log2FoldChange)>0.25 & ResA$baseMean>80), ]
...
genes2=rownames(vstLumRes)[which(vstLumRes$padj<0.05 & abs(vstLumRes$log2FoldChange)>0.20 & vstLumRes$baseMean>100)]
...
heatmap.2(assay(vsdLumvst)[ genes2,  ], scale="row", trace="none", ColSideColors = brewer.pal(4, "Greens")[ColSideCols], col=RdBu[11:1], main="Ep genes")
dev.off()
...
TCGAssgsea=gsva((TCGArsem2), list(grow=na.omit(HumGeneList2), 
                                                stab=Rat2Hum$HGNC.symbol[match(Glistnon, Rat2Hum$RGD.symbol)],
                                  Onco=Oncoprint[[1]], Mamma=Oncoprint[[2]]), method="ssgsea", ssgsea.norm=F)
...
Followed by the DFS plots for stable signature
...
newinfoTab=data.frame(pat=BrClin$Patient.ID[l1], TCGAval=TCGAssgsea[2, lx2],
                      TCGAstab=TCGAssgsea[1, lx2],
                      TCGAcomb=TCGAssgsea[2, lx2]-TCGAssgsea[1, lx2],
...
par(mfrow=c(2,2))
ax=cor.test(newinfoTab$TCGAcomb, newinfoTab$leuk, method="spearman")
smoothScatter(newinfoTab$TCGAcomb, newinfoTab$leuk)
text(0.6,0.6, sprintf("cor=%s, p=%s", round(ax$estimate, 2), round(ax$p.value, 2)))
ax=cor.test(newinfoTab$TCGAcomb, newinfoTab$str, method="spearman")
smoothScatter(newinfoTab$TCGAcomb, newinfoTab$str)
text(0.6,0.6, sprintf("cor=%s, p=%s", round(ax$estimate, 2), round(ax$p.value, 2)))
ax=cor.test(newinfoTab$TCGAcomb, newinfoTab$TCRshann, method="spearman")
smoothScatter(newinfoTab$TCGAcomb, newinfoTab$TCRshann)
...
summary(model3)
model3=coxph(SurvOS~TCGAgrow+Mamma+Onco+Stage, data=newinfoTab)
summary(model3)
```
...
summary(model3)
model3=coxph(SurvDFS~TCGAgrow+Mamma+Onco+Stage, data=newinfoTab)
summary(model3)
```
...
```{r TCGA-deseq}
newinfoTab$TCGAvalcut=cut(newinfoTab$TCGAstab,  quantile(newinfoTab$TCGAstab, c(0, 0.33, 0.67, 1) ), c("L", "M", "O"))
...
<!--chapter:end:09ac-associate_METABRIC.Rmd-->
...
rGene2=colSums(sign(rGeneOut2))/nrow(rGeneOut2)
names(rGene2)=toupper(names(rGene2))
```
...
image(Acast2, xaxt="none", yaxt="none", col=c("white", "red", "blue", "forestgreen"))
axis(1, at=seq(0, 1, length=nrow(Acast2)), rownames(Acast2), las=2)
axis(2, at=seq(0, 1, length=ncol(Acast2)), colnames(Acast2), las=2)

```


## Also look at the top RNA-mutations with are Ep specific

Look at the following set of genes which have been reported to be mutated in the epithelial samples based on RNAsequencing

```{r}
GeneList=c("Atm", "Tert", "Ncor2", "Epha3", "Arid1a", "Tp53", "Pten", "Pik3ca", "Pik3r1", "Esr1", "Ctcf", "Brca1", "Brca2", "Notch1", "Kmt2d", "Foxa1", "Gata3", "Mdm4", "Ccne1")

ax1=match(GeneList, colnames(Acast2))

image(Acast2[ ,ax1], xaxt="none", yaxt="none", col=c("white", "red", "blue", "forestgreen"))
axis(1, at=seq(0, 1, length=nrow(Acast2)), rownames(Acast2), las=2)
axis(2, at=seq(0, 1, length=ncol(Acast2)), colnames(Acast2), las=2)
...
Trichrome encapsulation

```{r}
df.Spatial$trichrome_pc[which(df.Spatial$trichrome_pc=="lots")]=1
df.Spatial$trichrome_pc
#wilcox.test(factor(as.numeric(df.Spatial$trichrome_pc))~df.Spatial$Growth)
```
...
#CNVGRanges=GRanges(seqnames = AllDCN$chrom, ranges=IRanges(start=AllDCN$start.pos, end=AllDCN$end.pos), copy_ratio=AllDCN$mean, call=AllDCN$call, sample=AllDCN2$sample)
