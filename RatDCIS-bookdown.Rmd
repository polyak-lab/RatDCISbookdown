...
```{r Ext5e-os, fig.cap="OS CD74 by subtype" }
par(mfrow=c(2,2))
...
with(TCGAdeseqRes, plot(log2FoldChange, -log10(padj), pch=20, main="Volcano plot: CD Stable (+) vs Growing (-)", cex=1.0, xlab=bquote(~Log[2]~fold~change), ylab=bquote(~-log[10]~Q~value)))
...
vstTCGA=vst(assay(TCGAdeseqRes))

ColSideCols=c("skyblue", "blue")[vstTCGA$TCGAvalcut]

ax1=assay(vstTCGA)[ rownames(TCGAdeseqRes2),  ]
ax1[1:5, 1:5]

pdf("figure-outputs/Figure5_XX_heatmap_LumA_TCGA.pdf", height=15, width=10)

heatmap.2(assay(vstTCGA)[ rownames(TCGAdeseqRes2),  ], scale="row", trace="none", ColSideColors = ColSideCols, col=RdBu[11:1], main="LumA TCGA")
...
p=ggplot(TrustTab[CD45F2, ], aes(x=Treatment, y=value, col=Treatment))+geom_boxplot()+geom_point()+
  scale_colour_manual(values=c(ColMerge[ ,1], "black"))+facet_grid(~variable)+theme_bw()
p+geom_text(data=ann_text, mapping=aes(x=2, y=0.75, label=Glabel))
