...
CD45ddsChar=DESeqDataSetFromMatrix(allstarFinal[ ,epidx], infoTableFinal[epidx, ], design=~factor(SizeCat)) ## change class
...
t2=assay(vsd)[which(rownames(vsd)%in%RatAllImm & rownames(vsd)%in%rownames(CD45res2)), match(colnames(CD45ddsChar), colnames(vsd))]
...
Markers3=c('Ar', 'Foxa1', 'Gata3', 'Erbb2', 'Esr1', 'Pgr', 'Krt5', 'Krt18', 'Vim', 'Tp53' )
...
Ax1=assay(vsd)[ match(Markers3, rownames(assay(vsd))), match(c2, colnames(vsd))]
Pam3=pam(t(Ax1), 2)
km3=kmeans(t(Ax1), 2)
...
heatmap.plus(Ax1, col=brewer.pal(11, "RdBu")[11:1], ColSideColors =CTable,scale="none", main="Char cohort vsd marker list1", trace="none")
...
```{r}
xb=sort(table(MeltWGSCoding$mutPos[MeltWGSCoding$Gene_Symbol%in%topCOSMIC]), decreasing = T)
...
WGSListgenes=table(MeltWGSCoding$Gene_Symbol)
...
WGSList=paste(MeltWGSCoding$Gene_Symbol, MeltWGSCoding$Start_Position)
t2=table(WGSList)
...
Wtemp=acast(MeltWGSCoding[which(MeltWGSCoding$mutPos%in%mutlist), c("L1", "mutPos", "Start_Position")],mutPos~L1)
...
ExprVSD=nVSDchar[na.omit(match(RNames, rownames(nVSDchar))), na.omit(match(colnames(Ttemp), colnames(nVSDchar)))]
...
pdf("~/Dropbox (Partners HealthCare)/Carlos' leukocyte project/Manuscript/figure_panels_generated_R.pdf", height=6, width=6)
