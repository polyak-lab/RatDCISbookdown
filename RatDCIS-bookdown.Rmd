...
write.csv(cbind(Cdata$Time.NMU2Sac[which(Cdata$Cohort=="Characterisation")], 
                ...
                Cdata$Char.Cohort.Batch[which(Cdata$Cohort=="Characterisation")]), file="nature-tables/Ext1c.csv")
```
...
#dev.off()
write.csv(t2temp, file="nature-tables/Ext1a-b.csv")
```
...
write.csv( GrowthRaw2, file="nature-tables/3b.csv")
```
...
#write.csv(WSIfracMelt, file="nature-tables/4b.csv")
...
d1=DNismr3$ProcessNetworks[, c("imm_vs_control NES", "stable_vs_growing padj")]
ds=which(d1[ ,2]<0.05)
...
# for (i in testType){
#   barplot(as.numeric(termA[which(TermType==i), 1]), names.arg = sigTerms[which(TermType==i)], horiz = T, las=2,
#           xlab="NES", main=testType, col=factor(sign(termA[which(TermType==i), 1])))
# }
...
write.csv(gscaepNI_lum@result$GSEA.results$ProcessNetworks, file="nature-tables/5g_lumxx.csv")
```

<!--chapter:end:09-EpCAM_comparison_inflammatory.Rmd-->

# Associate Epcam+ inflammatory with survival
...
```{r 5i, fig.cap="OS: growing signature"}
par(mfrow=c(2,2))
...
```{r 5i-dfs, fig.cap="DFS forgrowing signature"}
par(mfrow=c(2,2))
...
#pdf("figure-outputs/Fig5_TCGA_deg.pdf", height=8, width=8)
...
pdf("figure-outputs/Figure5_XX_heatmap_LumA_TCGA.pdf", height=15, width=10)
...
boxplot(vstTCGA["CD8A", ]~TCGAdeseq$TCGAvalcut)
...
write.csv(gscaTCGA@result$GSEA.results$ProcessNetworks, file="nature-tables/5XX_TCGA_lumA_GSEA.csv")
```
...
```{r Ext1-pca, fig.cap="PCA plot of epithelial samples"}
...
#colnames(vstB)=infoTableFinal$TumorIDnew[match(colnames(vstB), rownames(infoTableFinal))]
...
AllP=c(a1, b1, c1)
AllP=AllP[c(seq(2, 12, by=3), seq(1, 12, by=3), seq(3, 12, by=3))]
ann_text=data.frame(Glabel=round(AllP, 2), 
                    Treatment=rep(c("LY", "PDL1", "PDL1+LY"), 4), 
                    variable=rep(yvals, each=3), value=0.7)

print('P values for something')
a1
b1
c1
...
save(tablex, file="figure-outputs/figure-trust.Rdata")
ggplot(tablexM, aes(x=Treatment, y=value, col=Treatment))+geom_point()+facet_grid(variable~., scale="free_y", space="free_y")+theme_bw()
#+scale_colour_manual(values=c(ColMerge[ ,1], "black"))+theme_bw()
...
write.csv(TrustTab[CD45F2, ], file="nature-tables/4i_trust.csv")
```
...
write.csv(newTab, file="nature-tables/Ext5z_trichrome.csv")
...
# BICseqout
## Move to combined data load page
...
Create a TxDb object from a gtf file and save information 

```{r, cache=T, message=F, warning=F, eval=F}
txRn6=makeTxDbFromGFF("dontUpload/rn6_refGene.gtf",
...
```{r, eval=F}
AllCosmic=read.csv("anntotations/Census_COSMIC_Feb2020.csv")
RatCosmic=unique(na.omit(SymHum2Rat$RGD.symbol[match(AllCosmic$Gene.Symbol, SymHum2Rat$HGNC.symbol)]))
```

```{r, eval=F}
ImmPortGenes=read.csv("anntotations/ImmPort_Set.csv")
InnateDBGenes=read.csv("anntotations/innatedb_curated_genes.csv")

# convert to Rat genes
AllImmGenes=unique(toupper(c(as.character(ImmPortGenes$Symbol), as.character(InnateDBGenes$Gene.Symbol))))
RatAllImm=na.omit(SymHum2Rat$RGD.symbol[match(AllImmGenes, SymHum2Rat$HGNC.symbol)])
```
...
# load all and save the temporary outputs to file to see how the parameters change
# pdf("rslt/BICseq_summary_2scale.pdf", width=12, height=6)
# cnFreq_mod(AllDCN2,CN_low_cutoff=1.4, CN_high_cutoff=2.8, genome=cytoInfo)
# dev.off()
```
...
* Fig 3E: [Contingency table growth rate and treatment](#3e)
...
* Fig 4I: [trust-progression COHORT](#fig:4i)
...
* Fig 5F: [lum only non-inflamm growth vs stable DEG](#fig:ColSideCols)
...
* Ext Fig 1L: [PCA of growing vs stable epithelial cells](#fig:Ext1f)
...
* Ext2F [cNo fucking idea where this came from](#fig:Ext2f)
...
* Ext Fig 4C-D [WSI-normalised-all-samples](#fig:Ext4c) here \@ref(Ext4c)
* Ext Fig 4E [Knn with treatment](#fig:Fig4c) 
* Ext Fig 4F [Interacting fraction with treatment](#fig:if-growth) 
* Ext Fig 4H [MH with treatment](#fig:4h) 


## Extended data 5{-}

* Ext Fig5A [growth profiles of inflammatory tumors](#fig:5xta)
* Ext Fig 5B: [](#fig:Ext5b)
...
* Ext Fig 5E: [Survival plots TCGA and CD74 by subtype](#fig:Ext5e-os)
* Ext Fig 5F: [DEG all inflammatory removed](#fig:Epddsinflamm)
* Ext Fig 5G [non-inflammatory branch hairball](#fig:gsea-non-inflammatory-branch)
* Ext Fig 5I: [Trichrome association with inflammation and growth rate](#fig:)
* Ext Fig 5J: [Transcriptional heterogeneity](#fig:Ext5j)
