...
```{r}
Cdata=xlsx::read.xlsx("../metadata/Carlos_NEW_MASTER_2020-12-30.xlsx", sheetIndex=1)

## temp only
## add in info on size for the CXX samples
#ax1=which(is.na(infoTableFinal$TumSize))
#infoTableFinal$TumSize[ax1]=Cdata$Tumor.diameter.sac.mm[match(infoTableFinal$TumorID[ax1], Cdata$TumorID)]
...
boxplot(as.vector(Nx1)~X2, col=c(1:3), ylab="number of tumors per rat", xlab="Cohort")
...
#dev.off()
```
Plot the growth of tumours which u-turn:

```{r}
UturnSamp=c("6RB","11ND", "8LD", "10LD", "3NB" )

mx=match(UturnSamp, colnames(GrowthRaw))
...
d1$tum.size=Cdata$Tumor.diameter.sac.mm[match(rownames(d1), gsub("_", "", Cdata$TumorID))]
...
pdf(sprintf("~/Desktop/Figure3_summary_growth_plots_contigency%s.pdf", Sys.Date()), width=6, height=5)
...
```{r}
par(mfrow=c(1,3))
boxplot(as.numeric(Cdata$DN.Frac.FACS)~Cdata$Tumor.Growth, col=ColSizeb, ylab="DN")
boxplot(as.numeric(Cdata$CD45.Frac.FACS)~Cdata$Tumor.Growth, col=ColSizeb, ylab="CD45")
boxplot(as.numeric(Cdata$EpCAM.Frac.FACS)~Cdata$Tumor.Growth, col=ColSizeb, ylab="EpCAM")

par(mfrow=c(1,3))
boxplot(as.numeric(Cdata$DN.Frac.FACS)~Cdata$Treatment, col=ColMerge[ ,1], ylab="DN")
boxplot(as.numeric(Cdata$CD45.Frac.FACS)~Cdata$Treatment, col=ColMerge[ ,1], ylab="CD45")
boxplot(as.numeric(Cdata$EpCAM.Frac.FACS)~Cdata$Treatment, col=ColMerge[ ,1], ylab="EpCAM")
...
```{r, fig.hright=5}
pdf("~/Desktop/Fig2-WSI-summary-allsamples.pdf", height=5, width=7)
...
Is there a difference if we normalise to tumor size:

```{r}
WSIvalsTS=t(t(WSIvals)/Cdata$Tumor.diameter.sac.mm[match(colnames(WSIvals), gsub("_", "", Cdata$TumorID))])
WSIvalsTSmelt=melt(WSIvalsTS)

WSIvalsTSmelt$treatment=WSImelt$treatment
WSIvalsTSmelt$growth=WSImelt$growth

pdf("~/Desktop/Fig4B-summary-WSI-growth-treatment-norm-tumor-size.pdf", height=7, width=7)

nx1=levels(WSIvalsTSmelt$Var1)

nOut=sapply(nx1, function(x) wilcox.test(WSIvalsTSmelt$value[WSIvalsTSmelt$growth=="growing" & WSIvalsTSmelt$Var1==x],WSIvalsTSmelt$value[WSIvalsTSmelt$growth=="stable" & WSIvalsTSmelt$Var1==x])$p.value)

ggplot(WSIvalsTSmelt[-which(is.na(WSIvalsTSmelt$growth)), ], aes(x=Var1, y=value, col=factor(growth)))+geom_boxplot()+geom_point(position=position_jitterdodge(),alpha=0.3)+
  scale_color_manual(values=c(ColSize, "black"))+theme(axis.text.x = element_text(angle = 90, hjust = 1))+ggtitle("ordered by CD8")+theme_bw()+ggtitle(paste(names(nOut), round(nOut,2), collapse=" ;"))


nOutA=sapply(nx1, function(x) wilcox.test(WSIvalsTSmelt$value[WSIvalsTSmelt$treatment=="Vehicle" & WSIvalsTSmelt$Var1==x],WSIvalsTSmelt$value[WSIvalsTSmelt$treatment=="LY" & WSIvalsTSmelt$Var1==x])$p.value)

nOutB=sapply(nx1, function(x) wilcox.test(WSIvalsTSmelt$value[WSIvalsTSmelt$treatment=="Vehicle" & WSIvalsTSmelt$Var1==x],WSIvalsTSmelt$value[WSIvalsTSmelt$treatment=="PDL1" & WSIvalsTSmelt$Var1==x])$p.value)

nOutC=sapply(nx1, function(x) wilcox.test(WSIvalsTSmelt$value[WSIvalsTSmelt$treatment=="Vehicle" & WSIvalsTSmelt$Var1==x],WSIvalsTSmelt$value[WSIvalsTSmelt$treatment=="PDL1+LY" & WSIvalsTSmelt$Var1==x])$p.value)


ggplot(WSIvalsTSmelt, aes(x=Var1, y=value, col=(treatment)))+geom_boxplot()+geom_point(position=position_jitterdodge(),alpha=0.3)+
  scale_color_manual(values=ColMerge[ ,1])+theme(axis.text.x = element_text(angle = 90, hjust = 1))+ggtitle("ordered by CD8")+theme_bw()+
  ggtitle(paste(paste(names(nOutA), round(nOutA,2), collapse=" ;"),paste(names(nOutB), round(nOutB,2), collapse=" ;"),paste(names(nOutC), round(nOutC,2), collapse=" ;") ) )
...
par(mfrow=c(2,3))
for (i in tsamples){
  t1=wilcox.test(Cdata[Cdata$Pathologist.CD8.Spatial=="Infiltrating", i], Cdata[Cdata$Pathologist.CD8.Spatial=="restricted", i])$p.value
  boxplot(Cdata[, i]~Cdata$Pathologist.CD8.Spatial, main=sprintf("inf/rest: %s p=%s",i, round(t1,2)), xlab="", ylab=i)
# legend("topleft", paste(levels(Cdata$Growth2), round(t1, 2)))
}
```
...
par(mfrow=c(2,3))
for (i in tsamples){
  a1=cor.test(Cdata[ ,i],Cdata$Tumor.diameter.sac.mm/Cdata$Time.NMU2Sac, use="complete")
  #a1=cor.test(WSIvals[i, ],TumSizeNorm)
  plot( Cdata$Tumor.diameter.sac.mm/Cdata$Time.NMU2Sac.days, Cdata[ ,i],xlab="growth rate", ylab=i, main=sprintf("rho=%s, p=%s, %s",round(a1$estimate,2), round(a1$p.value,2), i))
}


par(mfrow=c(2,3))
for (i in tsamples){
  a1=cor.test(Cdata[ ,i],Cdata$Tumor.diameter.sac.mm, use="complete")
  #a1=cor.test(WSIvals[i, ],TumSizeNorm)
  plot( Cdata$Tumor.diameter.sac.mm,Cdata[ ,i], xlab="tumor size @ sac", ylab=i, main=sprintf("rho=%s, p=%s, %s",round(a1$estimate,2), round(a1$p.value,2), i))
}
...
# SummaryData$TumSize=Cdata$Tumor.diameter.sac.mm[lx]
...
# SummaryData$SpatialManual=Cdata$Pathologist.CD8.Spatial[lx]
...
# print(p)
p<-ggplot(knnTemp1, aes(col=Treatment, y=MeanDistance, x=Treatment))+geom_boxplot(outlier.shape=NA)+geom_point(position=position_jitterdodge(),alpha=0.3)+facet_grid(~knn+NearestCellType)+scale_y_continuous(trans='log10')+theme(axis.text.x = element_text(angle = 90, hjust = 1))+scale_color_manual(values = ColMerge[ ,1],na.value="black")+theme_bw()
print(p)
...
```{r}
pdf("~/Desktop/4C-knn-summary-growth.pdf", height=5, width=7)
...
A1$CD8frac=WSIvalFracs[1, match(A1$L1, colnames(WSIvalFracs))]
...
p=ggplot(IFmelt[IFmelt$Reference=="CD8" & IFmelt$knn=="knn3" & IFmelt$Dist==15 , ], aes(x=Growth2, y=value, col=Growth2))+facet_grid(~NearestNeighbor)+geom_boxplot()+ylab("Interacting Fraction")+xlab("Distance in microns")+ggtitle("Interacting fraction, knn3, dist=15")+theme_bw()+geom_point(position=position_jitterdodge(),alpha=0.3)+scale_color_manual(values=c(ColSize, "black"))
...
```{r}
pdf("~/Desktop/4E-MH-summary-growth.pdf", height=5, width=7)

p<-ggplot(A1, aes(x=gridsize, y=value, col=Growth2))+facet_grid(~Var1)+geom_line(aes(group=L1))+ylab("grid size")+ylab("Morisita Horn index")+ggtitle("MH index with CD8: growth")+geom_point(position=position_jitterdodge(),alpha=0.3)
print(p)
...
ggplot(A3, aes(x=Growth2, y=value, col=Growth2))+facet_grid(~Var1)+geom_boxplot()+ylab("grid size")+ylab("Morisita Horn index")+ggtitle("MH index with CD8: growth")+geom_text(data=ann_text, mapping = aes(x=2, y=0.75, label=label))+scale_color_manual(values=c(ColSize, "black"))
...
# 
ggplot(A2, aes(x=Var1, y=value, col=Growth2))+facet_grid(~Growth2)+geom_line(aes(group=L1))+ylab("grid size")+ylab(sprintf("Morisita Horn index @ %s", MHgridsize))+ggtitle(sprintf("MH index with CD8: stable p=%s, growing p=%s", round(a1$p.value, 2), round(a2$p.value, 2)))+geom_errorbar(aes(ymin=MH.lower, ymax=MH.upper), width=.2)+geom_point()

ggplot(A2, aes(x=Var1, y=value, col=Growth2))+facet_grid(~Growth2)+geom_line(aes(group=L1))+ylab("grid size")+ylab(sprintf("Morisita Horn index @ %s", MHgridsize))+ggtitle(sprintf("MH index with CD8: stable p=%s, growing p=%s", round(a1$p.value, 2), round(a2$p.value, 2)))+geom_errorbar(aes(ymin=MH.lower, ymax=MH.upper), width=.2)+geom_point()

A4=log2(A2t$value[seq(1, nrow(A2t), by=2)]/A2t$value[seq(2, nrow(A2t), by=2)])

boxplot(A4~A2t$Growth[seq(1, nrow(A2t), by=2)], main="EpCAM MH/ SMA MH")

t.test(A4~A2t$Growth[seq(1, nrow(A2t), by=2)])
dev.off()
```
...
pdf("~/Desktop/4E-MH-summary-matched-samples-treatment.pdf", height=5, width=7)
...
boxplot(A4~A2t$Treatment[seq(1, nrow(A2t), by=2)], main="EpCAM MH/ SMA MH")

#t.test(A4~A2t$Treatment[seq(1, nrow(A2t), by=2)])
...
for (i in 1:length(cNames)){
  ...
  if (length(xa)>0){
  ttestTable[i]=t.test( Cdata[ -xa,cNames[i]]~Cdata$Pathologist.CD8.Spatial[-xa])$p.value
  } else {
    ttestTable[i]=t.test( Cdata[ ,cNames[i]]~Cdata$Pathologist.CD8.Spatial)$p.value
  }
  boxplot(Cdata[ ,cNames[i]]~Cdata$Pathologist.CD8.Spatial, main=sprintf("%s p:%s", cNames[i], round(ttestTable[i],2)))
}
...
plot(log2(Cdata$IFEpstrRatio), log2(Cdata$MHEpstrRatio), col=Cdata$Pathologist.CD8.Spatial, xlab="IF", ylab="MH",ylim=c(-3, 3),
...
plot(log2(Cdata$IFEpstrRatio), log2(Cdata$knnEpstrRatio), col=Cdata$Pathologist.CD8.Spatial, xlab="IF", ylab="knn", 
      main=sprintf("ep to SMA fraction cor=%s p=%s", round(a1$estimate, 2), round(a1$p.value, 2)))
a1=cor.test(log2(Cdata$IFEpstrRatio+0.01), log2(Cdata$knnEpstrRatio), use="complete", na.rm=T)
plot(log2(Cdata$MHEpstrRatio), log2(Cdata$knnEpstrRatio), col=Cdata$Pathologist.CD8.Spatial, xlab="MH", ylab="knn", xlim=c(-4, 4),
      main=sprintf("ep to SMA fraction cor=%s p=%s", round(a1$estimate, 2), round(a1$p.value, 2)))

a1=cor.test(log2(Cdata$IFEpAntstrRatio+0.01), log2(Cdata$MHEpAntstrRatio+0.01), use="complete", na.rm=T)
plot(log2(Cdata$IFEpAntstrRatio), log2(Cdata$MHEpAntstrRatio), col=Cdata$Pathologist.CD8.Spatial, xlab="IF", ylab="MH",ylim=c(-3, 3),      main=sprintf("ep to SMA fraction cor=%s p=%s", round(a1$estimate, 2), round(a1$p.value, 2)))
a1=cor.test(log2(Cdata$IFEpAntstrRatio+0.01), log2(Cdata$knnEpAntstrRatio), use="complete", na.rm=T)
plot(log2(Cdata$IFEpAntstrRatio), log2(Cdata$knnEpAntstrRatio), col=Cdata$Pathologist.CD8.Spatial, xlab="IF", ylab="knn", 
      main=sprintf("ep to SMA fraction cor=%s p=%s", round(a1$estimate, 2), round(a1$p.value, 2)))
a1=cor.test(log2(Cdata$IFEpAntstrRatio+0.01), log2(Cdata$knnEpAntstrRatio), use="complete", na.rm=T)
plot(log2(Cdata$MHEpAntstrRatio), log2(Cdata$knnEpAntstrRatio), col=Cdata$Pathologist.CD8.Spatial, xlab="MH", ylab="knn", xlim=c(-4, 4),     main=sprintf("ep to SMA fraction cor=%s p=%s", round(a1$estimate, 2), round(a1$p.value, 2)))
...
#infoTable$TumorIDnew=Cdata$NewID[match(infoTable$TumorID, Cdata$TumorID)]
...
infoTableFinal$Treatment2=ifelse(infoTableFinal$Treatment=="Vehicle", "none", "imm")
...
CDdds=DESeqDataSetFromMatrix(allstarFinal[ ,cdidx], infoTableFinal[cdidx, ], design=~Treatment2+factor(Batch)) ## change class
...
plotPCA(vsdLimmaEp, c("Growth"))+scale_color_manual(values=c("orange", "#31A354"))+theme_bw()+ggtitle("Growth Ep")
plotPCA(vsdLimmaEp, c("Treatment"))+theme_bw()+ggtitle("Treatment Ep")+scale_color_manual(values=c(brewer.pal(3, "Oranges"), "#5D5D5D"))
...
plotPCA(vsdLimmaCD, c("Growth"))+scale_color_manual(values=c("orange", "#31A354"))+theme_bw()+ggtitle("Growth CD45")
plotPCA(vsdLimmaCD, c("Treatment"))+theme_bw()+ggtitle("Treatment CD45")+scale_color_manual(values=c(brewer.pal(3, "Oranges"), "#5D5D5D"))
...
plotPCA(vsdLimmaDN, c("Growth"))+scale_color_manual(values=c("orange", "#31A354"))+theme_bw()+ggtitle("Growth CDN")
plotPCA(vsdLimmaDN, c("Treatment"))+theme_bw()+ggtitle("Treatment DN")+scale_color_manual(values=c(brewer.pal(3, "Oranges"), "#5D5D5D"))
...
## Output: relate this to treatment
...
epr=results(EpddsTreatB)
Xa1=epr[which(epr$padj<0.05 & epr$baseMean>60), ]

epvsd=vst(EpddsTreatB)
...
Mat2=vsdLimmaDN[match(AllG, rownames(vsdLimmaDN)), ]
colnames(Mat2)=Cdata$NewID[match(substr(colnames(Mat2),1 , nchar(colnames(Mat2))-3), Cdata$TumorID)]
...
pdf("~/Desktop/Fig5_survival_analysis_Nicksamples_baseMean_greater100.pdf", height=8, width = 8)
...
pdf("~/Desktop/Fig5_survival_analysis_lowexpressing-included.pdf", height=8, width = 8)
...
# Progression cohort
Ax1=assay(vstEp)[ match(Markers3, rownames(assay(vstEp))),]
Pam1=pam(t(Ax1), 2)
plot(Pam1, main="narrow list, progression cohort")
ColA=Pam1$clustering
ColA=ifelse(ColA==1, "Basal", "Lum")

#Ax2=assay(vstEp)[ match(Markers3, rownames(assay(vstEp))),]
km1=kmeans(t(Ax1), 2)
clusplot(t(Ax1),km1$cluster, main="narrow list, progression cohort")
ColB=km1$cluster
ColB=ifelse(ColB==1, "Basal", "Lum")

table(ColA, ColB)

CTablea=cbind(ColA, ColB)
CTable=gsub("Basal", "red", CTablea)
CTable=gsub("Lum", "blue", CTable)

#pdf("~/Desktop/Fig3X-progression-cohort-hormone-receptor-status.pdf", height=5, width=6)
#heatmap(Ax1, col=brewer.pal(11, "RdBu")[11:1], scale="row", main="Progression cohort vsd marker list1", trace="none", distfun=function(x) (1-abs(cor(x))))
#dev.off()


colnames(Ax1)=infoTableFinal$TumorIDnew[match(colnames(Ax1), rownames(infoTableFinal))]

hclust.ave <- function(x) hclust(x, method="average")
ax1=heatmap.2(Ax1, trace="none", hclustfun = hclust.ave, col=brewer.pal(11, "RdBu")[11:1], scale="row")
...
Cdata$HR_status[match(colnames(CorMatEp), Cdata$NewID)]=ColSideCol

ax1=heatmap.2(Ax1, trace="none", hclustfun = hclust.ave, col=brewer.pal(11, "RdBu")[11:1], scale="row", ColSideColors = ColSideColB)
...
#table(ColC, ColD)
CTableb=cbind(ColC, ColD)
CTable=gsub("Basal", "red", CTableb)
CTable=gsub("Lum", "blue", CTable)
...
rownames(Ax1b)=infoTableFinal$TumorIDnew[match(rownames(Ax1b), rownames(infoTableFinal))]
heatmap.2(t(Ax1b), trace="none", hclustfun = hclust.ave, col=brewer.pal(11, "RdBu")[11:1], scale="row")
...
#Cdata$HR_status=NA
#Cdata$HR_status[match(substr(names(ColA), 1, nchar(names(ColA))-3), Cdata$TumorID)]=ColA
#Cdata$HR_status[match(substr(names(ColC), 1, nchar(names(ColC))-3), Cdata$TumorID)]=ColC
...
```{r}
Ax1=assay(vstDN)[ match(Markers2, rownames(assay(vstDN))),]
Pam5=pam(t(Ax1), 2)
...
km5=pam(t(Ax1), 2)
clusplot(t(Ax1),km5$cluster, main="narrow list, progression cohort")
ColF=km5$cluster
ColF=ifelse(ColF==1, "Basal", "Lum")


CTabled=cbind(ColE, ColF)
CTable=gsub("Basal", "red", CTabled)
CTable=gsub("Lum", "blue", CTable)

pdf("~/Desktop/5X-DN-samples-HR-status.pdf", width=6, height=5)

heatmap.plus(Ax1, col=brewer.pal(11, "RdBu")[11:1], ColSideColors =CTable,scale="none", main="DN samples vsd marker list1", trace="none")

nrm=which(colnames(Ax1)=="3L_D_DN")
heatmap.plus(Ax1[ ,-nrm], col=brewer.pal(11, "RdBu")[11:1], ColSideColors =CTable[-nrm,],scale="row", main="DN samples vsd marker list1", trace="none")
...
#a1=table(infoTableFinal$HR_pam[x1], Cdata$PgR..IF.[match(infoTableFinal$TumorID[x1], Cdata$TumorID)])
#ContTable(a1[ ,-2], "Pgr Expr", T, "Pgr IF", "compressed subtype")
...
# remove all the DN samples
#x1=which(infoTableFinal$Fraction=="Ep")
#a1=table(infoTableFinal$HR_pam[x1], Cdata$ER..IF.[match(infoTableFinal$TumorID[x1], Cdata$TumorID)])
...
Pall$PamCallshort=infoTableFinal$HR_pam[match(Pall$X, infoTableFinal$SampleID)]
...
```{r, eval=F}
Ax1=assay(vsd)[ match(Markers2, rownames(assay(vsd))),na.omit(midx)]
Ax1b=allRSEMnorm2log[na.omit(match(toupper(Markers2), rownames(allRSEMnorm2log))), na.omit(midx)]

Ax2=assay(vsd)[ match(Markers2, rownames(assay(vsd))),na.omit(midx2)]
Ax2b=allRSEMnorm2log[na.omit(match(toupper(Markers2), rownames(allRSEMnorm2log))), na.omit(midx2)]

Ax3=assay(vsd)[match(Markers2, rownames(assay(vsd))),na.omit(midx3)]
Ax3b=allRSEMnorm2log[na.omit(match(toupper(Markers2), rownames(allRSEMnorm2log))), na.omit(midx3)]
...
for (i in 1:nrow(Cancer_counts)){
  a1=glm(CCount2[i, ]~Fraction+Treatment+CD8Frac+MHEpCAM+GrowthRate+TumSize+HR_pam, data=infoTabTemp)
...
for (i in 1:nrow(Cancer_counts)){
  a1=glm(CCount2[i, infoTabTemp$Fraction=="DN"]~Treatment+CD8Frac+MHEpCAM+GrowthRate+TumSize+HR_pam, data=infoTabTemp[infoTabTemp$Fraction=="DN", ])
  MatrixSumm[i, ]=coefficients(a1)
  PValsSumm[i, ]=summary(a1)$coefficients[ ,4]
}
...
for (i in 1:nrow(Cancer_counts)){
  a1=glm(CCount2[i, infoTabTemp$Fraction=="Ep"]~Treatment+CD8Frac+MHEpCAM+GrowthRate+TumSize+HR_pam, data=infoTabTemp[infoTabTemp$Fraction=="Ep", ])
  MatrixSumm[i, ]=coefficients(a1)
  PValsSumm[i, ]=summary(a1)$coefficients[ ,4]
}
...
for (i in 1:nrow(Cancer_counts_dir)){
  a1=glm(Cancer_counts_dir[i, ]~Fraction+Treatment+CD8Frac+MHEpCAM+GrowthRate+TumSize+HR_pam, data=infoTabTemp)
  MatrixSumm[i, ]=coefficients(a1)
  PValsSumm[i, ]=summary(a1)$coefficients[ ,4]
}
...
for (i in 1:nrow(Cancer_counts_dir)){
  a1=glm(Cancer_counts_dir[i, infoTabTemp$Fraction=="DN"]~Treatment+CD8Frac+MHEpCAM+GrowthRate+TumSize+HR_pam, data=infoTabTemp[infoTabTemp$Fraction=="DN", ])
  MatrixSumm[i, ]=coefficients(a1)
  PValsSumm[i, ]=summary(a1)$coefficients[ ,4]
}
...
for (i in 1:nrow(Cancer_counts_dir)){
  a1=glm(Cancer_counts_dir[i, infoTabTemp$Fraction=="Ep"]~Treatment+CD8Frac+MHEpCAM+GrowthRate+TumSize+HR_pam, data=infoTabTemp[infoTabTemp$Fraction=="Ep", ])
  MatrixSumm[i, ]=coefficients(a1)
  PValsSumm[i, ]=summary(a1)$coefficients[ ,4]
}
...
cols.cor <- cor(Imm_counts, use = "pairwise.complete.obs", method = "pearson")
rows.cor <- cor(t(Imm_counts), use = "pairwise.complete.obs", method = "pearson")
hclust.col <- hclust(as.dist(1-cols.cor)) 
hclust.row <- hclust(as.dist(1-rows.cor))


heatmap.plus(Imm_counts, col=brewer.pal(11, "RdBu")[11:1], trace="none", ColSideColors =ColTable, scale="none", symkey=T, main="All CD45 samples", Colv = as.dendrogram(hclust.col),
          Rowv = as.dendrogram(hclust.row))

xidx=which(infoTableFinal$Cohort[grep("CD45", colnames(allTPMFinal))]=="Progression")
xidx

heatmap.plus(Imm_counts[ ,xidx], col=brewer.pal(11, "RdBu")[11:1], trace="none", ColSideColors =ColTable[ xidx, ], scale="none", symkey=T, main="All Progression CD45 samples")
```
...
Make the plots with individual samples:
...
image(cbind(CorV,CorV), col=RdBu[11:1], xaxt="none", yaxt="none",
        main="correlation CIBERSORT collapsed (* sig p<0.1)")
...
Plot the cibersort and/or EPIC output:
...
boxplot(TrustTab[CD45F2 ,c("TrustNclonotypes")]~TrustTab[CD45F2, "Treatment"], main=paste("PDL1", round(a1[1],2), "LY", round(b1[1],2), "PDL1+LY", round(c1[1], 2)), xlab="treat", ylab="no clonotypes",
        col=ColMerge[ ,1])
boxplot(TrustTab[CD45F2 ,c("TrustShannon")]~TrustTab[CD45F2, "Treatment"], main=paste("PDL1", round(a1[2],2), "LY", round(b1[2],2), "PDL1+LY", round(c1[2], 2)), 
        xlab="growth", ylab="shannon",col=ColMerge[ ,1])
boxplot(TrustTab[CD45F2 ,c("TrustGini")]~TrustTab[CD45F2, "Treatment"],main=paste("PDL1", round(a1[3],2), "LY", round(b1[3],2), "PDL1+LY", round(c1[3], 2)),   ylab="gini", xlab="growth",col=ColMerge[ ,1])
boxplot(TrustTab[CD45F2 ,c("TrustTop10")]~TrustTab[CD45F2, "Treatment"], main=paste("PDL1", round(a1[4],2), "LY", round(b1[4],2), "PDL1+LY", round(c1[4], 2)),   ylab="top10 clonotypes", xlab="growth", col=ColMerge[ ,1])

par(mfrow=c(2,2))
a1=sapply(yvals, function(x) wilcox.test(TrustTab[CD45F2 ,x]~TrustTab[CD45F2, "Growth"])$p.value)
boxplot(TrustTab[CD45F2 ,c("TrustNclonotypes")]~TrustTab[CD45F2, "Growth"], main=sprintf("Growth: BCR # clonotypes %s", round(a1[1], 2)), xlab="growth", ylab="no clonotypes", col=ColSizeb)
boxplot(TrustTab[CD45F2 ,c("TrustShannon")]~TrustTab[CD45F2, "Growth"], main=sprintf("Growth: BCR # shan %s", round(a1[2], 2)), xlab="growth", ylab="shannon", col=ColSizeb)
boxplot(TrustTab[CD45F2 ,c("TrustGini")]~TrustTab[CD45F2, "Growth"], main=sprintf("Growth: Gini %s", round(a1[3], 2)),
          ylab="gini", xlab="growth", col=ColSizeb)
boxplot(TrustTab[CD45F2 ,c("TrustTop10")]~TrustTab[CD45F2, "Growth"],  main=sprintf("Growth: Top10", round(a1[4], 2)),
         ylab="top10 clonotypes", xlab="growth", col=ColSizeb)

par(mfrow=c(2,2))
a1=sapply(yvals, function(x) wilcox.test(TrustTab[CD45F2 ,x]~TrustTab[CD45F2, "SpatialManual"])$p.value)
boxplot(TrustTab[CD45F2 ,c("TrustNclonotypes")]~TrustTab[CD45F2, "SpatialManual"], main=sprintf("Spatial: BCR # clonotypes %s", round(a1[1], 2)), xlab="spatialMan", ylab="no clonotypes")
boxplot(TrustTab[CD45F2 ,c("TrustShannon")]~TrustTab[CD45F2, "SpatialManual"], main=sprintf("SpatMan: BCR # shan %s", round(a1[2], 2)), xlab="spatialMan", ylab="shannon")
boxplot(TrustTab[CD45F2 ,c("TrustGini")]~TrustTab[CD45F2, "SpatialManual"], main=sprintf("SpatMan: BCR # gini %s", round(a1[3], 2)),   ylab="gini", xlab="spatialMan")
boxplot(TrustTab[CD45F2 ,c("TrustTop10")]~TrustTab[CD45F2, "SpatialManual"], main=sprintf("SpatMan: BCR # top 10  %s", round(a1[4], 2)),  ylab="top10 clonotypes", xlab="spatialMan")

par(mfrow=c(2,2))
TrustTab$MHEpCAMcut=ifelse(TrustTab$MHEpCAM> median(unique(TrustTab$MHEpCAM)), "high", "low")
table(TrustTab$MHEpCAMcut)
# a1=sapply(yvals, function(x) wilcox.test(TrustTab[CD45F2 ,x]~TrustTab[CD45F2, "MHEpCAMcut"])$p.value)
# boxplot(TrustTab[CD45F2 ,c("TrustNclonotypes")]~TrustTab[CD45F2, "MHEpCAMcut"], main=sprintf("MHepcam: BCR # clonotypes %s", round(a1[1], 2)), xlab="MHepcam", ylab="no clonotypes")
# boxplot(TrustTab[CD45F2 ,c("TrustShannon")]~TrustTab[CD45F2, "MHEpCAMcut"], main=sprintf("MHepcam: BCR # shan %s", round(a1[2], 2)), xlab="MHepcam", ylab="shannon")
# boxplot(TrustTab[CD45F2 ,c("TrustGini")]~TrustTab[CD45F2, "MHEpCAMcut"], main=sprintf("MHepcam: BCR # gini %s", round(a1[3], 2)),   ylab="gini", xlab="spatialMan")
# boxplot(TrustTab[CD45F2 ,c("TrustTop10")]~TrustTab[CD45F2, "MHEpCAMcut"], main=sprintf("MHepcam: BCR # top 10  %s", round(a1[4], 2)),  ylab="top10 clonotypes", xlab="MHepcam")
...
<!--chapter:end:10c-mutations-RNA.Rmd-->
...
colCols[match(names(which(Pam3$clustering==2)), colnames(nVSDchar))]="blue"
...
a1a=wilcox.test(df.Spatial$Trichrome~df.Spatial$Growth)$p.val
boxplot(df.Spatial$Trichrome~df.Spatial$Growth, main=sprintf("Growth vs Trichrome p=%s", round(a1a,2)))
...
<!--chapter:end:14-trichrome-staining.Rmd-->
