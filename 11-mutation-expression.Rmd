...
OutputplotFun=function(t2, scaleR="row", main="expression:scaled CD"){
ax1=heatmap.2(t2, scale = scaleR, col=RdBu[11:1], trace="none", main=main, Rowv = NA)
...
OutputplotFun(t2, "row", "Scaled CD")
OutputplotFun(t2, "none", "non-Scaled CD")
  
t2=nVSDchar[ na.omit(match(fgenes,rownames(nVSDchar))) ,grep("Ep", colnames(nVSDchar))]
OutputplotFun(t2, "row", "Scaled Ep")
OutputplotFun(t2, "none", "non-Scaled Ep")
...
### Immune specific expression?

```{r}

Midx=match(rownames(a2), unlist(strsplit(as.character(RNADNAsamples$WGS), ".final.bam")))
MatchSample=paste(RNADNAsamples$Rat_ID[Midx], RNADNAsamples$Location.x[Midx], "Ep", sep="_")
MatchSampleDN=paste(RNADNAsamples$Rat_ID[Midx], RNADNAsamples$Location.x[Midx], "DN", sep="_")

#mEpsamples=match(MatchSample, colnames(allTPMFinal))
mEpsamples=match(MatchSample, colnames(assay(vsd)))

#tempVSD=matrix(NA, ncol=ncol(a2), nrow=nrow(a2))
# not all of these match
idx1=match(colnames(a2),rownames(assay(vsd)))
tempVSD=t(assay(vsd)[ na.omit(idx1),na.omit(mEpsamples)])
rownames(tempVSD)=rownames(a2)[which(!is.na(mEpsamples))]
a1=setdiff(colnames(a2), colnames(tempVSD))
mat2=matrix(NA, ncol=length(a1), nrow=nrow(tempVSD))
colnames(mat2)=a1
tempVSD=cbind(tempVSD, mat2)
mat3=matrix(NA, nrow=length(which(is.na(mEpsamples))), ncol=ncol(tempVSD))
rownames(mat3)=rownames(a2)[which(is.na(mEpsamples))]
tempVSD=rbind(tempVSD, mat3)
tempVSD=tempVSD[match(rownames(a2), rownames(tempVSD)), match(colnames(a2), colnames(tempVSD))]
#tempVSDb=matrix(NA, ncol=ncol(a2), nrow=nrow(a2))
#tempVSDb[match(rownames(tempVSD), rownames(a2)), ]

#tempVSD=scale(tempVSD)
rownames(tempVSD)=paste(rownames(a2), "epcam", sep="-")
tempVSDscale=scale(tempVSD)

par(mar = c(4, 8, 4, 2),  xpd = TRUE)
image(t(tempVSDscale), col=RdBu[11:1], xaxt='n', yaxt='n')
...
Redo the above two plots, omitting the cases which do not have RNA data

```{r}
x1=rowSums(tempVSD, na.rm=T)

tempVSD3=tempVSDscale[which(x1!=0), ]
MutB=a2[which(x1!=0), ]
...
par(mar = c(4, 8, 4, 2),  xpd = TRUE)
image(t(tempVSD3), col=RdBu[11:1], xaxt='n', yaxt='n')
axis(1, at=seq(0, 1, length=ncol(tempVSD3)), colnames(tempVSD3), las=2, cex=0.7)
axis(2, at=seq(0, 1, length=nrow(tempVSD3)), rownames(tempVSD3), las=2, cex=0.7)
text(na.omit(List2[ ,2]-1)/(ncol(tempVSD3)-1), na.omit(List2[ ,1]-1)/(nrow(tempVSD3)-1) , "*")
