...
pvals=rep(NA, length(Candidates))
x1=table(m2$growth[which(m2$Var2==Candidates[1])])
for (i in 1:length(Candidates)){
  xa=prop.test(c(
    length(which(m2$value[which(m2$growth=="growing" & m2$Var2==Candidates[i])]>0)), 
    length(which(m2$value[which(m2$growth=="stable" & m2$Var2==Candidates[i])]>0))), x1)
   pvals[i]=xa$p.value
}
